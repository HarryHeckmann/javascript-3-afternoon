...
let myStrongest = monstersInYourPocket.filter((element, index, wholeArray) => {
  return wholeArray[index].CP > 200
})
...
let orderTotals = orders.map((val) =>{
  return (val.tax+1) * val.price
});
...
let bobsTotal = purchases.map(e => e.owner === "Bob" ? e.price : null).reduce((total, elem) => total+elem)
